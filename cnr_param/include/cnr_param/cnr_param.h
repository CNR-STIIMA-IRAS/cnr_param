...
#if ROS1_MODULE == 1
      {
        ret = cnr::param::ros::has(key, what);
      }
#else
      {
...
template <typename T>
inline bool get(const std::string& key, T& value, std::string& what, const T& default_val, const bool& implicit_cast_if_possible, 
                const std::vector<ModulesID>& module)
{
...
template <typename T>
inline bool get(const std::string& key, T& value, std::string& what, const bool& implicit_cast_if_possible, const std::vector<ModulesID>& module)
{
  UNUSED(key);
  UNUSED(what);
  UNUSED(value);
...
#if ROS1_MODULE == 1
      {
        ret = cnr::param::ros::get<T>(key, value, what, implicit_cast_if_possible);
      }
#else
      {
        what = "ROS1 module not available";
...
#if ROS2_MODULE == 1
      {
        ret = cnr::param::ros2::get<T>(key, value, what, implicit_cast_if_possible);
      }
#else
      {
        what = "ROS2 module not available";
...
#if MAPPED_FILE_MODULE == 1
      {
        ret = cnr::param::mapped_file::get<T>(key, value, what, implicit_cast_if_possible);
      }
#else
      {
        what = "MAPPED FILE MODULE not available";
...
#if ROS1_MODULE == 1
      {
        ret = cnr::param::ros::set<T>(key, value, what);
      }
#else
      {
        what = "ROS1 module not available";
