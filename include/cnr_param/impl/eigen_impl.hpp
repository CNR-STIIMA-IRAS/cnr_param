...
{
  if(!cnr::param::utils::resize(lhs, rhs.rows(),  rhs.cols()))
    return false;
  lhs = rhs;
  return true;
}
...
{
  if(!cnr::param::utils::resize(lhs, int(rhs.size()),  1))
    return false;
...
inline bool solve(Eigen::MatrixBase<Derived>& x, const Eigen::MatrixXd& A, const Eigen::VectorXd& b)
{
  ...
  else   if((A.rows()==A.cols()) && (cnr::param::utils::rank(A) == A.rows()))
  {
    _x = A.fullPivLu().solve(b);
  }
  else if(cnr::param::utils::rank(A) == std::min(A.rows(),A.cols()))
  {
  ...
  return (cnr::param::utils::copy_to_block(x, _x, 0, 0, _x.rows(),1) );
}
...
template<typename T>
inline std::string to_string(const T& m, bool transpose)
{
  UNUSED(transpose);
