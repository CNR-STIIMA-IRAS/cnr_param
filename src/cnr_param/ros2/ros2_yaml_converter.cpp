...
public:
  ArgParser(int argc, const char* const argv[])
    : generic_("Generic options", 160)
    , conversion_options_("Namespaces (shared memories)", 160)
    , cmdline_options_("Full List of the Options")
    , argc_(argc)
    , program_name_(argv[0])
  {
    try
    {
      ...
      conversion_options_.add_options()(
          "to-ros2-format,t", po::value<std::vector<std::string>>()->composing()->value_name("str"),
          "Convert the yaml files to the ROS2 format. The path is relative to the executable, or absolute if it starts "
          "with '/'")("from-ros2-format,f", po::value<std::vector<std::string>>()->composing()->value_name("str"),
                      "Convert the yaml files from the ROS2 format.  The path is relative to the executable, or "
                      "absolute if it starts with '/'")(
          "output-filenames,o", po::value<std::vector<std::string>>()->composing()->value_name("str"),
          "The name of the output file. If not specified, it is the input concat to '_autogererated.yaml'");
      ...
      if (vm.count("output-filenames"))
      {
        ...
        for (auto& of : ofns)
        {
          ...
          {
            std::cerr << "The output file " << _fp.string() << " should be created a the directory "
                      << _fp.parent_path() << " that does not exist." << std::endl;
        ...
        if (vm.count(opt))
        {
          ...
          for (std::size_t i = 0; i < pp.size(); i++)
          {
            ...
            if (!ok)
            {
              ...
              throw po::validation_error(po::validation_error::invalid_option_value, "to-ros2-format", p);
            }
            ...
            if (opt == std::string("to-ros2-format"))
            {
              ...
              YAML::Node leaf;
              if (cnr::param::ros2::ros2_yaml_encoder(node, leaf, what))
              {
                std::cerr << what << std::endl;
                throw po::validation_error(po::validation_error::invalid_option_value, "to-ros2-format", p);
              }
              new_node["/**"]["ros__parameters"] = leaf;
            }
            else
            {
              ...
              YAML::Node _leaf;
              if (cnr::param::ros2::ros2_yaml_decoder(_node["ros__parameters"], _leaf, what))
              {
                std::cerr << what << std::endl;
                throw po::validation_error(po::validation_error::invalid_option_value, "from-ros2-format", p);
              }
              new_node = _leaf;
            }
            ...
            else
            {
              ...
              std::cout << "saving the converted file to '" << rawname + "_autogenerated.yaml"
                        << "'" << std::endl;
